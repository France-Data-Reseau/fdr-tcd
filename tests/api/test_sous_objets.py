...
def test_contact_creation_liaison_par_nom(client):
    """Dette v1 : projets des contacts = NOM du projet (champ Text)."""
    connecter(client, "editeur@exemple.fr")
    page = client.get("/projet/10/contact/nouveau?collectivite_id=5")
    assert "Ville Exemple" in page.text
    client.post(
        "/projet/10/contact/nouveau",
        data={
            "prenom": "Marie", "nom": "Martin", "elu": "1",
            "fonction": "Maire", "email": "marie@ville.fr", "telephone": "",
            "mobile": "", "collectivite_id": "5", "action": "retour",
            "csrf_token": extraire_csrf(page.text),
        },
    )
    fake = client.fake_grist
    cree = next(
        r for r in fake.records["BDD_Contacts"] if r.get("nom") == "Martin"
    )
    assert cree["projets"] == "Projet Lampadaires"
    assert cree["collectivite"] == 5
    assert cree["elu"] is True


def test_contact_email_invalide_refuse(client):
    connecter(client, "editeur@exemple.fr")
    page = client.get("/projet/10/contact/nouveau?collectivite_id=5")
    reponse = client.post(
        "/projet/10/contact/nouveau",
    ...
    assert "email valide" in reponse.text
