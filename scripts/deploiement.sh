#!/usr/bin/env bash
# Déploiement de la V2 vers le VPS (fdr2.revorun.eu) DEPUIS UN MAC/Unix.
...
echo "--- Vérifications ---"
sleep 8
V2="$(curl -s https://fdr2.revorun.eu/health)"
echo "fdr2.revorun.eu/health : $V2 (attendu $SHA)"
echo "$V2" | grep -q "$SHA" || { echo "ERREUR : version inattendue en production"; exit 1; }
echo -n "fdr.revorun.eu (v1) : "
curl -sI https://fdr.revorun.eu | head -1
