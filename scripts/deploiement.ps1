# Déploiement de la V2 vers le VPS (app.example.org) — BUILD SUR LE VPS,
...
$v2 = Invoke-RestMethod https://app.example.org/health
Write-Host "app.example.org/health : version $($v2.version) (attendu $sha)"
if ($v2.version -ne $sha) { throw "Version inattendue en production" }
$v1 = Invoke-WebRequest -UseBasicParsing -Method Head https://app-v1.example.org
Write-Host "app-v1.example.org (v1) : HTTP $($v1.StatusCode) — toujours en vie"
