# Déploiement — app.example.org (v1 intacte)
...
Conforme à la procédure de déploiement VPS documentée en interne. Règles absolues : ne jamais toucher au service
`fdr`, ne jamais exposer de port public supplémentaire, jamais de `docker compose down -v`.

## Séquence

1. **Avant tout** : Victor crée le DNS A `app.example.org` → <IP_DU_VPS>
...
4. **`.env` sur le VPS** (`~/stack/app_fdr_v2/.env`, chmod 600) :
   ...
   - variables magic link (**vitales** — vérifiées au démarrage) : `SMTP_HOST/PORT/USER/
     PASSWORD/FROM`, `APP_PUBLIC_URL=https://app.example.org` ;
...
7. **Vérifications post-déploiement** :
   ...
   curl -s https://app.example.org/health   # 200 + SHA attendu → LA BONNE version répond (R1)
   curl -I https://app-v1.example.org       # la v1 doit TOUJOURS répondre
   ```
