...
- **pytest + pytest-asyncio** (config dans `pyproject.toml`).
- **pygrister mocké** dans les tests de repositories/services (pattern des
  `test_*_repository.py` : mock de `GristApi`, vérification des
...
- e2e Playwright : hors périmètre initial, ajoutable ensuite
