...
1. **Architecture en couches (repositories / services / api)** — priorité n°1.
...
- Ne jamais toucher à l'app v1, au service Docker `fdr`, ni à `app.example.org`.
...
2. **Expéditeur des magic links** : Gmail (dev seulement) vs sous-domaine dédié de
   `example.org` + relais transactionnel SPF/DKIM (recommandé pour la prod) — qui configure ?
