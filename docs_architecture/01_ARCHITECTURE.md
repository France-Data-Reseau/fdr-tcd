...
- **Routes** (`app/api/`) : extraction du formulaire → modèle Pydantic → appel service →
  ...
  selon les bonnes pratiques en couches — exception : la restitution est `async def` (elle
...
- **Injection** (`app/dependencies.py`) : fabriques `get_*_repository()` / `get_*_service()`
  ...
  / `require_ownership`), selon le pattern d'injection standard.
...
|---|---|---|
| Framework | FastAPI + Jinja2 + uvicorn (Python 3.11) | Iso-v1 : templates réutilisés tels quels ; stack du repo de référence. |
| Client Grist | **pygrister 0.9.x** | Choix de Victor ; c'est la lib du repo de référence → pattern repository identique. Synchrone : compensé (voir Performance). `requests.Session` injectée pour timeouts + retries bornés. |
| Config | pydantic-settings (`core/config.py`) | Comme la référence. Tout en `.env`, y compris l'URL Grist (en dur dans la v1 — corrigé). **Refus de démarrer en prod sans `SECRET_KEY`**. |
...
| Lint/typage | ruff + pyright + pre-commit | Comme la référence ; critère d'acceptation. |
...
```
app_fdr_v2/
├── pyproject.toml               — deps prod+dev (uv), config ruff/pytest (calqué sur la référence)
├── uv.lock                      — versions verrouillées
├── .python-version              — 3.11
├── .pre-commit-config.yaml      — ruff + pyright avant commit (comme la référence)
...
│   │   ├── types.py             — TypedDicts des records des 12 tables BDD_* (comme la référence)
...
│
├── templates/                   — les 15 templates v1 repris tels quels (base, login, inscription,
